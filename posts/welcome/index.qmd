...
for (bk in seq(0,1,0.01)){
  second < bk
}

cut(second,breaks=seq(-1,1,0.01))
mean(mle[second < 0.18 & second>0.17]^2)
var(mle[second < 0.18 & second>0.17])
var(mle[second < 0.177 & second>0.173])
var(mle[second < 0.2 & second>0.1])

ct <- seq(0,1,0.1)
vv <- c()
for (i in 2:length(ct)){
  if (i ==2 ){v <- var(mle[second <-ct[i] & abs(second) > ct[i-1]])}
  else {v <- var(mle[second <-ct[i] & second > ct[i-1]])}
  vv <- c(vv,v)
}
